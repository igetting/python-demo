...
def dense_bilstm(input, nclass):
    _dropout_rate = 0.2
    _weight_decay = 1e-4

    _nb_filter = 64
    # conv 64 5*5 s=2
    x = Conv2D(_nb_filter, (5, 5), strides=(2, 2), kernel_initializer='he_normal', padding='same', use_bias=False,
               kernel_regularizer=l2(_weight_decay))(input)
    # x = Conv2D(_nb_filter, (3, 3), strides=(2, 1), kernel_initializer='he_normal', padding='same',use_bias=False, kernel_regularizer=l2(_weight_decay))(input)
    ...
    lstm_1 = LSTM(256, return_sequences=True, kernel_initializer='he_normal', name='lstm1')(x)  # (None, 32, 512)
    ...
    y_pred = Dense(nclass, name='out', activation='softmax')(lstm_merged)
